...
def getTableRows(args, contract, scope, table):
    ...
    rows = []
    while(more):
        tableInfo = getJsonOutput(args.focli + 'get table %s %s %s --limit 100 --index 1 -L %s' % (str(contract), str(scope), str(table), nextKey))
...
def getTableRow(args, contract, scope, table, pk):
    pkStr = str(pk)
    tableInfo = getJsonOutput(args.focli + 'get table %s %s %s --index 1 -L %s -U %s --key-type i64' % (str(contract), str(scope), str(table), pkStr, pkStr))
...
def startWallet():
    run('killall fowal || true')
    sleep(1.5)
    run('rm -rf ' + os.path.abspath(args.wallet_dir))
    run('mkdir -p ' + os.path.abspath(args.wallet_dir))
    background(args.fowal + ' --unlock-timeout %d --http-server-address 127.0.0.1:6666 --wallet-dir %s' % (unlockTimeout, os.path.abspath(args.wallet_dir)))
    sleep(.4)
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.focli + 'create account flon ' + a + ' ' + args.public_key)
...
#             ret = getJsonOutput(args.focli + 'system claimrewards -j ' + row['owner'])
...
#         if has_rewards :
#             voter = row['owner']
#             ret = getJsonOutput(args.focli + 'push action -j flon.reward claimrewards \'["' + voter + '"]\' -p ' + voter)
#             times.append(ret['processed']['elapsed'])
#     print('Elapsed time for voter claimRewards:', times)

# def updateAuth(account, permission, parent, controller):
#     run(args.focli + 'push action flon updateauth' + jsonArg({
#         'account': account,
...
def regAccount(acct, ramFunds, cpuFunds, netFunds):
    ...
    if args.check_account_existed and runStatus(args.focli + 'get account flon ' + acct['name']):
        return
    retry(args.focli + 'system newaccount --transfer flon %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' %
...
def stepInstallSystemContracts():
    retry(args.focli + 'set contract flon.token ' + args.contracts_dir + '/flon.token/')
    retry(args.focli + 'set contract flon.msig ' + args.contracts_dir + '/flon.msig/')
    retry(args.focli + 'set contract flon.reward ' + args.contracts_dir + '/flon.reward/')
    retry(args.focli + 'set account permission flon.reward active --add-code')

def stepCreateTokens():
    run(args.focli + 'push action flon.token create \'["flon", "1000000000.00000000 %s"]\' -p flon.token' % (args.symbol))
    # run(args.focli + 'push action flon.token issue \'["flon", "%s", "memo"]\' -p flon' % intToCurrency(totalAllocation))
    # allocate 90% of totalSupply
    run(args.focli + 'push action flon.token issue \'["flon", "900000000.00000000 %s", "memo"]\' -p flon' % (args.symbol))
    sleep(1)
def stepSetSystemContract():
    ...
    # or flon.system
    retry(args.focli + 'set contract flon ' + args.contracts_dir + '/flon.boot/')
    sleep(3)

    # activate remaining features
    # ACTION_RETURN_VALUE
    retry(args.focli + 'push action flon activate \'["c3a6138c5061cf291310887c0b5c71fcaffeab90d5deb50d3b9e687cead45071"]\' -p flon@active')
    # CONFIGURABLE_WASM_LIMITS2
    retry(args.focli + 'push action flon activate \'["d528b9f6e9693f45ed277af93474fd473ce7d831dae2180cca35d907bd10cb40"]\' -p flon@active')
    # BLOCKCHAIN_PARAMETERS
    retry(args.focli + 'push action flon activate \'["5443fcf88330c586bc0e5f3dee10e7f63c76c00249c87fe4fbf7f38c082006b4"]\' -p flon@active')
    # GET_SENDER
    retry(args.focli + 'push action flon activate \'["f0af56d2c5a48d60a4a5b5c903edfb7db3a736a94ed589d0b797df33ff9d3e1d"]\' -p flon@active')
    # FORWARD_SETCODE
    retry(args.focli + 'push action flon activate \'["2652f5f96006294109b3dd0bbde63693f55324af452b799ee137a81a905eed25"]\' -p flon@active')
    # ONLY_BILL_FIRST_AUTHORIZER
    retry(args.focli + 'push action flon activate \'["8ba52fe7a3956c5cd3a656a3174b931d3bb2abb45578befc59f283ecd816a405"]\' -p flon@active')
    # RESTRICT_ACTION_TO_SELF
    retry(args.focli + 'push action flon activate \'["ad9e3d8f650687709fd68f4b90b41f7d825a365b02c23a636cef88ac2ac00c43"]\' -p flon@active')
    # DISALLOW_EMPTY_PRODUCER_SCHEDULE
    retry(args.focli + 'push action flon activate \'["68dcaa34c0517d19666e6b33add67351d8c5f69e999ca1e37931bc410a297428"]\' -p flon@active')
     # FIX_LINKAUTH_RESTRICTION
    retry(args.focli + 'push action flon activate \'["e0fb64b1085cc5538970158d05a009c24e276fb94e1a0bf6a528b48fbc4ff526"]\' -p flon@active')
     # REPLACE_DEFERRED
    retry(args.focli + 'push action flon activate \'["ef43112c6543b88db2283a2e077278c315ae2c84719a8b25f25cc88565fbea99"]\' -p flon@active')
    # NO_DUPLICATE_DEFERRED_ID
    retry(args.focli + 'push action flon activate \'["4a90c00d55454dc5b059055ca213579c6ea856967712a56017487886a4d4cc0f"]\' -p flon@active')
    # ONLY_LINK_TO_EXISTING_PERMISSION
    retry(args.focli + 'push action flon activate \'["1a99a59d87e06e09ec5b028a9cbb7749b4a5ad8819004365d02dc4379a8b7241"]\' -p flon@active')
    # RAM_RESTRICTIONS
    retry(args.focli + 'push action flon activate \'["4e7bf348da00a945489b2a681749eb56f5de00b900014e137ddae39f48f69d67"]\' -p flon@active')
    # WEBAUTHN_KEY
    retry(args.focli + 'push action flon activate \'["4fca8bd82bbd181e714e283f83e1b45d95ca5af40fb89ad3977b653c448f78c2"]\' -p flon@active')
    # WTMSIG_BLOCK_SIGNATURES
    retry(args.focli + 'push action flon activate \'["299dcb6af692324b899b39f16d5a530a33062804e41f09dc97e9f156b4476707"]\' -p flon@active')
    # GET_CODE_HASH
    retry(args.focli + 'push action flon activate \'["bcd2a26394b36614fd4894241d3c451ab0f6fd110958c3423073621a70826e99"]\' -p flon@active')
    # GET_BLOCK_NUM
    retry(args.focli + 'push action flon activate \'["35c2186cc36f7bb4aeaf4487b36e57039ccf45a9136aa856a5d569ecca55ef2b"]\' -p flon@active')
    # CRYPTO_PRIMITIVES
    retry(args.focli + 'push action flon activate \'["6bcb40a24e49c26d0a60513b6aeb8551d264e4717f306b81a37a5afb3b47cedc"]\' -p flon@active')
    # BLS_PRIMITIVES2
    retry(args.focli + 'push action flon activate \'["63320dd4a58212e4d32d1f58926b73ca33a247326c2a5e9fd39268d2384e011a"]\' -p flon@active')
    # DISABLE_DEFERRED_TRXS_STAGE_1 - DISALLOW NEW DEFERRED TRANSACTIONS
    retry(args.focli + 'push action flon activate \'["fce57d2331667353a0eac6b4209b67b843a7262a848af0a49a6e2fa9f6584eb4"]\' -p flon@active')
    ...
    retry(args.focli + 'push action flon activate \'["09e86cb0accf8d81c9e85d34bea4b925ae936626d00c984e4691186891f5bc16"]\' -p flon@active')
    # SAVANNA
    # Depends on all previous protocol features
    retry(args.focli + 'push action flon activate \'["cbe0fafc8fcc6cc998395e9b6de6ebd94644467b1b4a97ec126005df07013c52"]\' -p flon@active')
    sleep(1)
...
def stepInitSystemContract():
    run(args.focli + 'push action flon init' + jsonArg(['0', '4,' + args.symbol]) + '-p flon@active')
    sleep(1)


def stepInitEvmContract():
    retry(args.focli + 'set contract flon.evm ' + args.contracts_dir + '/evm_runtime -p flon.evm@active')
    retry(args.focli + 'set account permission flon.evm active  --add-code -p flon.evm@active')
    params = {
        "chainid":15555,
        "fee_params": {
    ...
    run(args.focli + 'push action flon.evm init' + jsonArg(params) + '-p flon.evm@active')

    run(args.focli + 'transfer flon flon.evm "%s" "flon.evm" -p flon@active' % (fundToCurrency(1.0)))
    ...
    run(args.focli + 'transfer flon flon.evm "%s" "%s" -p flon@active' % (fundToCurrency(100.0), evm_pub_key.to_address()))
    run(args.focli + 'push action flon.evm open' + jsonArg(["evm.miner"]) + '-p evm.miner@active')
    sleep(1)
...
def stepRegAccounts():
    ...
    for p in accounts['producers']:
        ...
        retry(args.focli + 'system regproducer ' + p['name'] + ' ' + p['pub'] + ' https://flonscan.io/account/' + p['name'] + ' ' + str(reward_shared_ratio))
    sleep(1)
    # reg voters
...
def allocateVotes(voters):
    ...
    for i in range(0, voterLen):
        ...
        retry(args.focli + 'transfer flon %s "%s"' % (voterName, intToCurrency(staked)))
        retry(args.focli + 'push action flon addvote ' + jsonArg([voterName, intToCurrency(votes)]) + ' -p ' + voterName + '@active')
...
def voteProducers(voters, producers):
    ...
    for v in voters:
        ...
        prodNames.sort()
        retry(args.focli + 'push action flon voteproducer %s -p %s@active' % (jsonArg([v["name"], "", prodNames]), v["name"]))
...
commands = [
    ('k', 'kill',               stepKillAll,                False,    "Kill all fonod and fowal processes"),
...
parser.add_argument('--focli', metavar='', help="focli command", default='focli --wallet-url http://127.0.0.1:6666 ')
parser.add_argument('--fowal', metavar='', help="Path to fowal binary", default='fowal')
...
parser.add_argument('--url', metavar='', help="Http url for fonod", default='http://127.0.0.1:8888')
